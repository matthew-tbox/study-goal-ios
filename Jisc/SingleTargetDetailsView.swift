    ...
    func view(_ view: CustomPickerView, selectedRow: Int) {
        ...
        if (selectedRow == 0) {
            ...
            if(!dataManager.developerMode){
                urlString = "https://api.datax.jisc.ac.uk/sg/log?verb=viewed&contentID=logs-timed&contentName=logTimed&modid=\(module)"
            } else {
                urlString = "https://api.x-dev.data.alpha.jisc.ac.uk/sg/log?verb=viewed&contentID=logs-timed&contentName=logTimed&modid=\(module)"
            }
            ...
            var urlString = ""
            if(!dataManager.developerMode){
                urlString = "https://api.datax.jisc.ac.uk/sg/log?verb=viewed&contentID=logs-untimed&contentName=logReport&modid=\(module)"
            } else {
                urlString = "https://api.x-dev.data.alpha.jisc.ac.uk/sg/log?verb=viewed&contentID=logs-untimed&contentName=logReport&modid=\(module)"
            }
            xAPIManager().checkMod(testUrl:urlString)
        }
    }
